...
import { useMessageContext } from "../../../provider/Message/MessageDataProvider";
...
export default memo(function ({
  ...
  const renderMessageItem = ({ item }: { item: UserMessage }) => {
    const isMyMessage = item.uid === auth.user?.uid;
  
    return (
      <View key={item.id} style={[styles.messageItemContainer, isMyMessage ? styles.myMessageContainer : styles.otherMessageContainer]}>
        {isMyMessage ? (
          <Text fontWeight="bold" size="sm" style={{ marginBottom: 8, marginLeft: 0 }}>
            Me
          </Text>
        ) : (
          <Text fontWeight="bold" size="sm" style={{ marginBottom: 8, marginLeft: 0 }}>
